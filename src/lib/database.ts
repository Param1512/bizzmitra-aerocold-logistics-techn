...
export const DOMAIN_SCHEMA = {
  ...
  appTitle: "AeroCold Logistics Technologies",
...
const STORAGE_KEY = 'bizzmitra-aerocold-logistics-techn_db_logistics_v1';
const USERS_STORAGE_KEY = 'bizzmitra-aerocold-logistics-techn_users_logistics_v1';
const SPRINTS_STORAGE_KEY = 'bizzmitra-aerocold-logistics-techn_sprints_logistics_v1';
const ACTIVE_SESSION_KEY = 'bizzmitra-aerocold-logistics-techn_session_logistics_v1';
...
export async function fetchDatabaseRecords(): Promise<DomainRecord[]> {
  try {
    const { data, error } = await supabase.from('logistics_records').select('*');
    if (!error && Array.isArray(data) && data.length > 0) {
      const mapped: DomainRecord[] = data.map((d: any) => ({
        ...
        status: d.status || 'Manifest Created',
...
export async function persistRecord(item: DomainRecord, existingRecords: DomainRecord[]): Promise<DomainRecord[]> {
  ...
  try {
    await supabase.from('logistics_records').insert({
      id: item.id,
      title: item.title,
      col1_data: item.col1,
      col2_data: item.col2,
...
export async function updateRecordStatus(id: string, status: string, records: DomainRecord[]): Promise<DomainRecord[]> {
  ...
  try {
    await supabase.from('logistics_records').update({ status }).eq('id', id);
  } catch (e) {}
